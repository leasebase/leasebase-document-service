...
WORKDIR /app/service
COPY leasebase-document-service/package.json leasebase-document-service/package-lock.json* ./
RUN npm ci
COPY leasebase-document-service/tsconfig.json ./
COPY leasebase-document-service/src ./src
RUN npm run build
